...
<div class="top-bar" data-bind="visible:shownSection() == 'job-editor', with: job">
  <div style="margin-top: 4px; margin-right: 20px" class="pull-right">
    <a title="${_('Back to jobs list')}" href="#jobs"><i class="icon-reorder"></i> ${_('Sqoop Dashboard')}</a>
  </div>
  <!-- ko ifnot: $root.job().persisted -->
  <h4>${_('Sqoop Job Creation Wizard')}</h4>
  <!-- /ko -->
  <!-- ko if: $root.job().persisted -->
  <h4>${_('Sqoop Job Editor: ')}<span data-bind="editableText: $root.job().name" id="job-editor-name" contenteditable="true"></span></h4>
  <!-- /ko -->
</div>

<div class="top-bar" data-bind="visible:shownSection() == 'connection-editor', with: editConnection">
  <div style="margin-top: 4px; margin-right: 20px" class="pull-right">
    <a href="#connection/edit-cancel">${_('Back to job')}</a>
  </div>

  <!-- ko ifnot: persisted -->
  <h4>${_('Sqoop Connection Creation Wizard')}</h4>
  <!-- /ko -->
  <!-- ko if: persisted -->
  <h4>${_('Sqoop Connection Editor')}</h4>
  <!-- /ko -->
</div>
          ...
          <li data-bind="routie: 'job/edit/' + id()" title="${ _('Click to edit') }">
            ...
            <div class="main">
              <h4 style="display: inline-block">
                <i class="icon-list"></i> <span data-bind="text: type"></span> ${ _('from') } ${ _('to') } <span class="muted" data-bind="text: name"></span>
              </h4>
            </div>
